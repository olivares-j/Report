...
Tables \ref{tab:BayesFactors3deg} and \ref{tab:BayesFactors6deg} summarise the evidences and Bayes factors of the radially symmetric models using the candidate members within  7 pc (3$^{\circ}$) and 14 pc (6$^{\circ}$), respectively. In the Bayesian model selection methodology, the boundaries for decision making from Bayes Factors should be set \emph{ab initio}. Thus, we discuss our results following the classical scale by \cite{Jeffreys61}, which I shown in Table \ref{tab:JeffreysScale}.
...
\begin{table}[ht]
  \centering
      \caption[]{Natural logarithm of the evidence for each
        ...
        corresponds to the model including parameters for the cluster
        centre and a radial truncation at 7 pc (3$^{\circ}$).}
...
\begin{table}[ht]
 ...
 \caption[]{Natural logarithm of the evidence for each
        ...
        column header placed in the denominator). The evidence
        corresponds to the model including parameters for the cluster
        centre and a radial truncation at 14 pc (6$^{\circ}$).}
...
\caption{Samples of the posterior distribution of the King's profile parameters inferred from the 3$^{\circ}$ sample. }
\label{fig:King_7}
\end{center}
\end{figure}
...
For the RGP model, its MAP estimates are $\alpha_c=56.6909785^{\circ}$, $\delta_c=24.13301438^{\circ}$, $R_b=886.84$ pc , $\alpha=1.44$, and $\beta=70.6$. The corresponding standard deviation matrix $\Sigma$ is
...
It is evident from $\Sigma$ that $R_b$ and $\beta$ are only mildly constrained. The profile is, again, characterised by a cusp in the central part of the cluster, as is the profile of the GKing alternative. 

The 6$^{\circ}$ data set favours clearly MAP profiles (models and MAP estimates of their parameters) with sharp peaks in the centre of the cluster. However, we caution the reader against these results. They refer to a data set that is severely biased and incomplete in ways that we cannot quantify and/or model.
...
\hline              
           $3^{\circ}$-King    &  -2806.9   &  $>100$   &    0.14   &           &            &               \\
           $3^{\circ}$-OGKing  &            &   -1527.3 & $<0.01$   &           &            &               \\   
           $3^{\circ}$-RGP     &            &           &  -2808.9  &           &            &               \\
           \hline
           $6^{\circ}$-King    &            &           &           &  -5023.8  &  $>100$    &   $<0.01$     \\
           $6^{\circ}$-OGKing  &            &           &           &           &  -3255.7   &   $<0.01$     \\   
           $6^{\circ}$-RGP     &            &           &           &           &            &  -5034.7      \\
\hline              
         \end{tabular}
   \end{table}
...
\begin {figure}
 \centering
  \includegraphics[width=0.8\textwidth]{background/Figures/PSD/KingEll7.pdf}
  \caption{Samples from the posterior distribution of the parameters in the biaxial symmetric King model inferred from the 3$^{\circ}$ sample.}
\label{fig:KingEll7}
\end {figure}
...
We include luminosity ($J$ band) segregation in the radial and biaxial symmetric extensions of the King profile, which has shown earlier has both large Bayes factor and physical interpretability. Table \ref{tab:BayesFactorsLumSeg} summarises the evidences and Bayes Factors for these models, whose parameters are inferred from the datasets out to 3$^\circ$ and 6$^\circ$.

\begin{table}
  \centering
  \caption[]{ Evidences and bayes factors for radial and biaxial symmetric models incorporating luminosity segregation. }
...
\hline              
 $3^{\circ}$-King-Radial    &  -2875.5   &           &           &            \\
 $3^{\circ}$-King-Biaxial &            &  -2803.9  &           &            \\
 $6^{\circ}$-King-Radial    &            &           &   -5025.4 &            \\
 $6^{\circ}$-King-Biaxial &            &           &           &  -5016.4   \\
\hline              
         \end{tabular}
   \end{table}

Figure \ref{fig:KingMS7} shows the posterior distributions of the biaxial symmetric King model with luminosity segregation  applied on the 3$^{\circ}$ sample. This model is the best one according to our analysis. 

\begin {figure}
 \centering
  \includegraphics[width=0.8\textwidth]{background/Figures/PSD/KingMS7.pdf}
  \caption{Samples from some of the posterior distribution of the parameters in the biaxial symmetric King luminosity segregated model inferred from the 3$^{\circ}$ sample. Only shown the main axis of the biaxial model.}
\label{fig:KingMS7}
\end {figure}

The resulting evidences and Bayes Factors clearly indicate that for the Kings models inferred in both  3$^{\circ}$  and 6$^{\circ}$ data sets, there is strong evidence in favour of the more complex models with J-band luminosity segregation and biaxial symmetry (see Table \ref{tab:BayesFactorsAll}).

\begin{table}
  \centering
  \caption[]{Evidences and bayes factors for King models with radial symmetry (King-R), biaxial symmetry (King-B), and radial and biaxial symmetry with luminosity segregation (King-RS and King-BS, respectively). These evidences are computed using the samples of $3^{\circ}$ and $6^{\circ}$ }
...
\hline              
\multirow{4}{*}{$3^{\circ}$} &King-R&-2876.2&&&&&&&\\
&King-B&&-2806.9&&&&&&\\
&King-RS    & & &-2875.5   & &          &           &    &        \\
 &King-BS &    &   &     &  -2803.9  &   &&        &           \\
 \hline
\multirow{4}{*}{$6^{\circ}$}&King-R   && &&            & -5033.5          &   & &            \\
&King-B&&&&&&-5023.8&&\\
&King-RS &&&&            &           &         & -5025.4  &     \\
&King-BS &&&&            &           &         &  &  -5016.4   \\
\hline              
...
\begin{figure}[htp]
\begin{center}
\resizebox{\hsize}{!}{\includegraphics{background/Figures/BHM/M45vsM42vsM44.pdf}}
...
\begin{figure}[htp]
\begin{center}
\resizebox{\hsize}{!}{\includegraphics{background/Figures/BHM/CDF_comparison.pdf}}
