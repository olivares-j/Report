...
In this section I will review four of the MCMC categories: Metropolis-Hasting (MH), Hybrid Monte Carlo (HMC), ensemble samplers, and Nested Sampling. The MH category comprises the classic MH algorithm but also contain particular cases like the Gibbs sampler \citep{Geman1984}. I describe MH only for completeness and explanatory reasons. For the other three categories I will focus the particular cases of Hamiltonian Monte Carlo, for (HMC), affine invariant for ensemble samplers, and MultiNest for Nested Sampling. These three had properties which make them suitable for my objective.

By far, the most popular MCMC algorithm is Metropolis-Hastings \citep{Metropolis1953,Hastings1970}. Given the original, $\theta_o$, and proposed, $\theta_p$ positions of the Markov chain, which live in the state space, the chain moves from $\theta_o$ to $\theta_p$ with acceptance probability

\begin{equation}
\mathcal{A}(\theta_p|\theta_o)=min\left\{1,\frac{p(\theta_p)\cdot q(\theta_o|\theta_p)}{p(\theta_o)\cdot g(\theta_p|\theta_o)}\right\}.
\end{equation}
...
The Hybrid Monte Carlo algorithms use additional information about the target distribution to improve the acceptance ratio. Here, I focus only on Hamiltonian Monte Carlo  
