...
I use a modified version of \emph{CosmoHammer} \citep{Akeret2013}, a front-end of \emph{emcee}. This enables us to control the input and output of data and parameters, as well as the hybrid parallel computing. For this last one, instead of using OpenMP as \citep{Akeret2013} did, we use the \emph{multiprocessing} package of python. It  distributes the computing of the likelihood among cores in each cluster node. I run this code on a 80 CPUs (cores) computer cluster with 3.5 GHz processors.
...
However, since MPI (Message Passing Interface) was already used by \emph{emcee} to distribute the the particles across cores. I realised a hybrid approach was necessary. In this hybrid HPC approach, \emph{emcee} particles were distributed in nodes while objects were distributed across cores in a node. 
