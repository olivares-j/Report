...
\begin{figure}[ht!]
    ...
    \end{subfigure}
\caption{Scaled density distributions in $Y,J,H$ and $K_s$ bands of \gls{rdr2} objects with missing values in the rest of the bands. The features are similar to those in Fig. \ref{fig:NAsDDR2}. The small differences come from the fact that the \gls{rdr2} is not a random sample of the \gls{ddr2}.}
\label{fig:NAs}
\end{figure}

Truncation occurs when the data set does not contain records, neither of the measured value nor of the number of measurements, for objects whose measured quantity lies outside certain limits.\textbf{ The \gls{ddr2} is truncated at the sensitivity limits of the UKIDSS survey}. On the other hand, censoring happens when the data set contains only partial information about the actual value of the measured quantity. Basically, censoring happens when the value lies outside the upper or lower limits of the measuring instrument, and these limits become the only available record of the measurement.\textbf{ In the \gls{ddr2} censoring occurs in objects whose values are outside the sensitivity limits of the detectors.} Currently, the \gls{ddr2} does not provide upper or lower limits for the censored data. Although these limits could also be inferred from the data, the heterogeneous origin of the \gls{ddr2} prohibits this task. Furthermore, the missing values in the \gls{ddr2} data set do not occur only because of censoring. Indeed, cosmic rays, hot pixels, halos of bright stars, diffraction patterns and cross-matching failures are just some examples of potential sources of missing values. The statistical treatment of missing values originating from each of these sources lies beyond the scope of this work. 

However, in terms of probability, there is no distinction between missing values and parameters. Therefore, we can marginalise missing values as we do with any other nuisance parameter\textbf{ with the aid of a prior}. If the vector of measurements $\mathbf{d}$ has a missing value in entry $i$, then $x_{i} = NA$ , and $\mathbf{d}=\{x_1,x_2,...,x_{i-1},NA,x_{i+1}...,x_n\}$, with $n$ the dimension of $\mathbf{d}$. Thus, the marginalised likelihood of this datum, given model parameters $\boldsymbol{\theta}$ is

\begin{equation}
\label{eq:marginalmiss}
p(\mathbf{d}|\boldsymbol{\theta})= \int_{-\infty}^{\infty} p(\{x_1,x_2,...,x_{i-1},x_{i}= NA,x_{i+1},...,d_n\}|\boldsymbol{\theta})\cdot p(x_i)\cdot \mathrm{d}x_{i},
\end{equation}
\textbf{with $p(x_i)$ the prior distribution for the missing value. Because missing values, as explained before, arise not only from censoring but from several other sources, some of them unknown, I assume that the prior $p(x_i)$ is uniform. This assumption is called \emph{ignorability} of the missing process \cite[see Chapter 8 of][for more details]{Gelman2013}. Notice however, that the impact of the uniform prior relays deeply in the multidimensionality of the data and in the correlation between the missing and observed variables. When this correlation is negligible, the observed data provides no information, thus rendering the results similar to those performed only in the completely observed data. Since photometric variables are highly correlated (see Fig. \ref{fig:fphGMM2}), objects with missing values provide useful information that must be taken into account.}

\textbf{The modelling of missing values could be improved by the inclusion of more \emph{ad hoc} priors or selection functions, however, the uniform prior allows to perform the above integral analytically. Specifying a prior for each of the missing values would demand to numerically evaluate the integral, which would have required a tremendous amount of computing power.}
Therefore, throughout this work, missing entries are marginalised in the previous way whenever present.

%I made a remark of a point that may seem obvious but it is important to remember. Let $p(a)$ be a probability distribution, $A$ a random sample of $n$ points from it, and $p_A(a)$ the empirical probability distribution of $A$. Let $B$ be a non-random sample of $p(a)$ with $n$ elements, and empirical probability distribution $p_B(a)$. In the limit of $n\rightarrow \infty$, $p_A(a)=p(a)$ however $p_B(a)\neq p(a)$. Therefore, $p_A(a)\neq p_B(a)$. Similarly, in data with missing values, if the missing value pattern is not random, the distribution of the completely observed data (with non-missing values) differs from that of all the data. In \citet{Olivares2017} we show this subtle but important difference in the case of the \gls{dance} data set. 
...
In this Section, I present the methodology applied to investigate the \glsfirst{psd}. This analysis uses the candidate members in the \glsfirst{hmps} (see Section \ref{sect:RDR2}), the ones recovered by the \gls{bhm}. Although the analysis of the \gls{psd} uses the membership probabilities of the \gls{bhm}, it is independent of the latter. It means that the kinematic and photometric distributions derived by the  \gls{bhm}  are assumed to be independent of the \gls{psd} of the cluster. Section \ref{sect:RF-2} details the reasons of this decision. 

The generative model of the \gls{psd} is similar to that of the \gls{bhm}  (Eq. \ref{eq:genmod}). However, instead of inferring the fraction of field objects in the data set (which would be similar to the estimated contamination rate on the \gls{hmps}, see Section \ref{sect:classifier}), each object contributes to the cluster model proportionally to its membership probability, $P$ (the ones obtained by the \gls{bhm}). Thus the generative model of the \gls{psd} for object $n$ is
...
\end{equation}
 ...
 This algorithm preserves the monotonic convergence of the conventional \gls{em}, and returns positive definite matrices provided that $\rho < 2$. For details and validation of this algorithm see \citet{McMichael1996}.
