...
Let $y=g(x)$ be the nonlinear transformation, with $g$ a function of $x$ with inverse $g^{-1}$, continuous and with continuous derivative\footnote{It can be continuous by parts, resulting in a continuous by parts PDF, e.g. a $n$ power-law distribution.}, so that $x=g^{-1}(y)$, with $y\subset \mathbb{R}$. Then, the following is true,
...
Finally, before ending this Section, I present a summary of the symbols used to represent the parameters in the BHM (Table \ref{table:symbols_parameters}), together with its graphical representation in the form of a PGM (Fig. \ref{fig:PGMBHM}).
