...
From the statistical perspective, it is important to know if their origin is deterministic or stochastic. In the former, they occur only above or below a certain hard limit, while in the latter, they follow a probability distribution. If its origin is deterministic, there are two possible scenarios: censoring and truncation. 
...
In the following, whenever missing values appear in the likelihood functions, they will be marginalised using the previous equation.
Furthermore, I assume that the probability $p(I|\mathbf{d})$ is uniform. This assumption is called \emph{ignorability} because the result of it is that $p(\mathbf{d}_{obs},I|\boldsymbol{\theta})= p(\mathbf{d}_{obs}|\boldsymbol{\theta})$, which is equivalent to ignores the missing data mechanism.  Although I acknowledge that this assumption is incorrect (missing values are most probable at the faint photometric ends, see Section \ref{sect:DR2}), I am obliged to assume it because of the following reasons.

%Because missing values arise not only due to censoring but from several other sources, many of them unknown, I assume that the missing data collection mechanism can be ignored, which essentially is the same as adopting a uniform prior for  $p(I|\mathbf{d})$. Thus 
%\begin{equation}
%p(\mathbf{d}_{obs}|\boldsymbol{\theta})=\int p(\mathbf{d}|\boldsymbol{\theta})\mathrm{d}\mathbf{d}_{mis}. \nonumber
%\end{equation}
%This assumption is called \emph{ignorability} of the data collection mechanism \footnote{
%Following \citet{Gelman2013} two conditions are sufficient to ensure ignorability.
%
%\emph{Distinct parameters} requires independence between the model parameters $\theta$ and the missing data process parameters $\phi$. This means
%\begin{equation}
%p(\phi|\theta)=p(\phi). \nonumber
%\end{equation}
%\emph{Missing at random} requires that the probability of having a missing value only depends in the observed values. This is
%\begin{equation}
%p(I|y,\phi)=p(I|y_{obs},\phi), \nonumber
%\end{equation}
%Then, the joint posterior distribution of the model and missing mechanism parameters,
%\begin{align}
%p(\theta,\phi|y_{obs},I) \propto p(\theta,\phi) p(y_{obs},I|\theta,\phi) =p(\theta,\phi)\int p(y,I|\theta,\phi)\mathrm{d} y_{mis}=p(\theta,\phi)\int p(I|y,\phi)p(y|\theta) \mathrm{d} y_{mis}\nonumber
%\end{align}
%marginalised over $\phi$, 
%\begin{align}
%p(\theta|y_{obs},I)=p(\theta)\int \int p(\phi|\theta) p(I|y,\phi)p(y|\theta) \mathrm{d} y_{mis} \mathrm{d}\phi, \nonumber
%\end{align}
%and using the previous conditions, results in 
%\begin{align}
%p(\theta|y_{obs},I)=p(\theta)\int \int p(\phi) p(I|y_{obs},\phi)p(y|\theta) \mathrm{d} y_{mis} \mathrm{d}\phi =p(\theta)\int p(y|\theta)\mathrm{d} y_{mis}=p(\theta|y_{obs}).\end{align}
%Thus, proving the ignorability of the missing data mechanism.
%}.


%I decided to ignore the data collection mechanism for the following reasons.
First, the $p(I|\mathbf{d},\boldsymbol{\theta})$ is not known. The \gls{ddr2} is a compilation of data from a variety of surveys, thus the data collection mechanism in neither homogeneous not (currently\footnote{ The DANCe team is currently working on the assessing of the missingness process. In particular,  we are interested in the detection probability as a function of signal-to-noise and sky position. The latter is of paramount importance in the vicinity of bright sources where halos and artefacts have the largest impact.}) totally understood. 

Second, even if a more elaborate and accurate probability $p(I|\mathbf{d},\boldsymbol{\theta})$ would be available, the current computational capabilities will not allow to numerically evaluate the integral of Eq. \ref{eq:marginalmis}. The use of a uniform probability enable us to analytically evaluate this integral thus avoiding the high computational burden of computing it for each object and missing value in the data set and at each step in the MCMC chain.

Since this assumption is not correct, it inevitably results in biased estimators of the true population parameters. In the following Section I quantify this bias.

\subsubsection{Validity of the \emph{ignorability} assumption.}
\label{sect:ignorability}

In this Section, I present the quantification of the bias introduced by assuming the \emph{ignorability} of the missing-value mechanism. In addition, I also quantify the bias introduced by assuming that the true population parameters can be recovered using only completely observed objects and discarding those with missing entries. I call the latter the \emph{naivety} assumption. Notice that these two assumptions will be undistinguishable in the absence of correlation amongst observables. 

To quantify the biases rendered by these assumptions, I proceed as follows.

First, I assume to know the parameter values of the true population. I the parameter values of the \gls{gmm} describing the photometry of the field population contained in the \gls{rdr2} data set. In specific, I assume that these true parameter values are those inferred in Section \ref{sect:field_population}. 

Second, using these true parameter values I generate a synthetic data set of size equal to that of the \gls{rdr2}.

Third, using the real data from the \gls{rdr2}, I mask as missing the photometric entries in the synthetic data. The masking procedure is the following.
...
Thus, I now have three synthetic data sets. The original one, with completely observed entries, the one with missing values, and, the one resulting from taking only the completely observed objects from the one with missing values. To each of these data sets, I apply the \gls{gmm} algorithm (the one described in Section \ref{sec:codeGMM}) and recover the fractions, means and covariance matrices of the \gls{gmm}. I call these set of parameters original, ignorable, and naive, in accordance to the data set from which they were obtained.

By means of these three set of parameters, I can quantify the bias induced by the \emph{ignorability} and \emph{naivety} assumptions. Furthermore, to minimise the effects of the sample size in the comparisons, instead of comparing the ignorable and naive set of parameters to the true parameters, I do use the original set of parameters. To quantify differences I use the absolute relative difference, defined as the absolute value of the relative difference between the ignorable and naive parameters to the original ones. Figure \ref{fig:ignorable_and_naive} shows the absolute relative differences in the projected densities (in the set of \glspl{cmd} used in this work) between the ignorable and naive \glspl{gmm}. As can be seen, the naive parameters return projected densities that over-estimate the density at the central parts, where $\gls{ci}\sim4$, whereas the ignorable parameters return densities similar to the original ones, except at the bright end, where it also over-estimate the original ones.
...
\end{subfigure}
\caption{Contours comparing the absolute relative difference between the naive and original models (left panels), and the ignorable and the original ones (right panels) for each of the \glspl{cmd} used in this work. The densities resulting of the original model are shown by means of the grey scale. Regions where the density of the original model goes below $10^{-4}$ are not shown.}
\label{fig:ignorable_and_naive}
\end{center}
\end{figure}


In addition, the mean absolute relative difference between the ignorable and naive, and the original, set of parameters are $0.19 \pm 0.2$ and $1.7 \pm 3.8$, respectively. These figures indicate that the ignorability assumption is indeed biased by a mean 19\%, whereas the naivety assumption is biased by an astonishing 170\%. 

The modelling of missing values could be greatly improved by removing the ignorability assumption and using correct prior probabilities for the missing-data mechanism. These priors are expected to be available in future releases of the DANCe survey, together with detailed information of the upper and lower limits of each missing value.

Nevertheless, the \emph{ignorability} assumption (i.e. the uniform prior), which enable us to perform the marginalisation integral (Eq. \ref{eq:marginalmis}) analytically an thus reduce the computing time, renders better estimates of the true parameters than the naivety assumption used by the previous worksin the literature (see Section \ref{sect:current_methodologies}). 
...
In this Section, I present the methodology applied to investigate the \glsfirst{psd}. The data set for this analysis is described in Section \ref{}. Here suffices to mention that is comprises the largest and less contaminated sample of Pleiades candidate members to date.  Although the analysis of the \gls{psd} uses the membership probabilities of the \gls{bhm} (derived in this work) and those of \citet{Bouy2015} for the Tycho-2 sample, it is independent of both the latter because both works explicitly removed the sky positions in their analyses. See Section \ref{sect:RF-2} details the reasons of this decision. 

The generative model of the \gls{psd} is similar to that of the \gls{bhm}  (Eq. \ref{eq:genmod}). However, instead of inferring the fraction of field objects in the data set (which would be similar to the estimated contamination rate on the \gls{hmps}, see Section \ref{sect:classifier}), each object contributes to the cluster model proportionally to its membership probability, $P$ (the ones obtained by the \gls{bhm} and by \citealt{Bouy2015}). Thus the generative model of the \gls{psd} for object $n$ is
...
\begin{equation}
\rho(R)=\rho(0)\cdot(1+(R/A)^2)^\frac{\gamma}{2}.
\label{eq:eff}
\end{equation}
...
%The new likelihood term is based on the empirical distribution function and is expressed as
%
%\begin{equation}
%\mathcal{L}(\bm{\phi}) = p(\bm{\phi}|\bm{\theta}) = \prod_{n=1}^{N}\mathcal{N}(\phi_n|\mu=n\cdot2\pi/N,\sigma)
%\end{equation}
%
%where $\bm{\phi}$ is the vector of ordered angles $\phi_n\in {0,2\pi}$ calculated for a given centre and coordinate system (parameters included in the set $\bm{\theta}$); $\phi_n$ is the n-th angle in the vector $\bm{\phi}$, and $\sigma$ is the allowed level of departure from the cumulative distribution function of the uniform distribution. 
...
where the parameter $R_b$ follows the same elliptic angle dependence as defined in Equations \ref{eq:anglerc} and \ref{eq:anglert}, and we do not incorporate any angle dependence for the exponents $\alpha$ and $\beta$.

\subsection{Models with luminosity segregation}
\label{sect:SegregatedModels}
In this Section, we consider an extension to introduce a dependence of the core radius with the $J$ magnitude. We select the $J$ magnitude because it is the \textbf{bluest} of the magnitudes that are available for \textbf{almost} all candidate members in the \gls{hmps} (\textbf{see Table \ref{tab:hmps}}). We assume that stars of the same mass have approximately the same magnitude and that distance differences (due to the 3D spatial extend of the Pleiades) average out. The core radius dependence with the $J$ magnitude is modelled as
