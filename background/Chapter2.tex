...
The photometric completeness of the \gls{ddr2} is difficult to estimate due to its heterogeneous origin (e.g. see Figure \ref{fig:originDANCeDR2}). Nevertheless, in the following I will attempt to estimate it. To do it, in the following I will assume that the distribution of sources in the region of the sky occupied by the Pleiades grows as a power-law in the observed magnitude of the sources. This assumption is of course very simplistic since the cluster itself and the galaxy prevent this exact law in the increasing number of objects as a function of the magnitude. Despite this reason, this assumption suffices for the objectives of the present work. As can be seen in Figure \ref{fig:completeness}, the distribution of sources in the \gls{ddr2} as a function of the photometric magnitudes grows almost like a power-law. At certain limits, the power-law behaviour stops and the number of sources rapidly falls. In the following, I will assume that the survey is complete below this drop-off limit.
...
Thus, to avoid such biases, I assume that the \gls{ukidss} survey \citep{2007MNRAS.379.1599L}, which is the most sensitive from among those that contribute to the \gls{ddr2} data set, provides the homogeneous spatial and sensitivity coverage at faint magnitudes (see Fig. \ref{fig:originDANCeDR2}, and \citealt{Bouy2013}). According to \citet{2007MNRAS.379.1599L}, the completeness limits of the \gls{ukidss} survey (which are shown in Figure \ref{fig:completeness} as vertical dotted lines) are: $Y\sim20.16$, $J\sim19.56$,$H\sim18.81$, and $K\sim18.19$ magnitudes. In addition, \citet{Bouy2015} estimate the the completeness limit of the \gls{ddr2} in the $i$ band is $i\sim23$ mag. However, as can be seen from Figure \ref{fig:completeness}, such limits are optimistic for the \gls{ddr2}. After re-estimating them, I find the following values: $i\sim21.2$, $Y\sim19.2$, $J\sim18.6$,$H\sim18$, and $K\sim17.4$ magnitudes, which are shown in Figure \ref{fig:completeness} as the vertical dashed lines. 

\begin{figure}[htbp]
\begin{center}
\resizebox{\hsize}{!}{\includegraphics[width=0.5\textwidth]{background/Figures/MagnitudeDistributionsDDR2.pdf}}
\caption{Density of all \gls{ddr2} sources as a function of the observed magnitudes. The vertical dotted and dashed lines correspond to the completeness limits of the literature, and to those derived as the mode of the densities, respectively. The bump in the $i$ distribution at 15.5 mag corresponds to the completeness limits of the APASS survey.  See text and \citet{Bouy2015} for more details.}
\label{fig:completeness}.
\end{center}
\end{figure}

The \gls{ddr2} is also incomplete in the bright end due to saturation of the detectors. The bright end of the $J,H,K$ bands is given by the 2MASS survey, thus the observed values of this bands in the \gls{ddr2} fall within the sensitivity limits of the 2MASS detectors. On the other hand the $Y$ and $i$ bands are prone to saturation, as can be seen in Figure \ref{fig:completeness} for the $Y$ band. The case of the $i$ band is more intricate. 

The $i$ band in the \gls{ddr2} comes from two main surveys: APASS in the bright end (completeness limit at 15.5 mag., see bump in Figure \ref{fig:completeness}), and CFHT/MegaCam (yellow pattern in Figure \ref{fig:originDANCeDR2}) at the mid and low ends (with completeness limit at $i\sim21.2$). However, while APASS covers 100\% of the \gls{ddr2} sky area and its detectors do not saturates at the bright end (at least within the range covered by the \gls{ddr2} values), the detectors of CFHT/MegaCam, which covers just the central $3^\circ$ area, saturate at $i\sim13$, as indicated by \citet{Bouy2015}. Due to this intricate pattern, I will adopt the lower completeness limit given by the latter authors.

As I will show in Chapter \ref{chap:BHM},  we are interested in the completeness limits of the $i -K_s$ colour index, which are defined by those of $i$ and $K_s$ together. In Fig. \ref{fig:completenessK+i}, I show the $K_s$ and $i$ 2d kernel density estimate of the \gls{ddr2} sources. This density shows a sharp decline below $i=13.2$ mag., which most probably is the product of saturation, as mentioned before. At this value of $i$ band, I chose the value of $K_s=11.0$ mag as a conservative lower completeness limits. Therefore, the central rectangle of Figure \ref{fig:completentessK+i} depicts the region where the $i-K_s$ colour index is assumed to be complete.
...
As can be seen from this figure, the \gls{t+d} catalogue is complete until magnitude $J\sim19$ and radial distance of 11.5 pc ($\sim5^\circ$). We notice that the latter corresponds roughly with the sky coverage of the UKIDSS survey \citep{2007MNRAS.379.1599L}.
